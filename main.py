...
def train_one_dataset(params, file_name, train_q_data, train_qa_data, train_pid,train_ans,valid_q_data, valid_qa_data, valid_pid,valid_ans):
    ...
    for idx in range(params.max_iter):
        ...
        train_loss, train_accuracy, train_auc = train(
            model, params, optimizer, train_q_data, train_qa_data, train_pid,train_ans,  label='Train')
        # Validation step
        valid_loss, valid_accuracy, valid_auc = test(
            model,  params, optimizer, valid_q_data, valid_qa_data, valid_pid, valid_ans,label='Valid')
...
def test_one_dataset(params, file_name, test_q_data, test_qa_data, test_pid, test_ans, best_epoch):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default="ednet_pid")
    ...
    train_data_path = params.data_dir + "/" + \
                      params.data_name + "_train"+str(params.train_set)+".txt"
    valid_data_path = params.data_dir + "/" + \
                      params.data_name + "_valid"+str(params.train_set)+".txt"

    train_q_data, train_qa_data, train_pid,train_ans = dat.load_data(train_data_path,'train')
    valid_q_data, valid_qa_data, valid_pid ,valid_ans= dat.load_data(valid_data_path,'valid')
    ...
    best_epoch = train_one_dataset(
        params, file_name, train_q_data, train_qa_data, train_pid,train_ans,valid_q_data, valid_qa_data, valid_pid,valid_ans)
    print("test")
    test_one_dataset(params, file_name, test_q_data,
                     test_qa_data, test_index,test_ans, best_epoch)
