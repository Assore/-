...
def train(net, params,  optimizer,  q_data, qa_data, pid_data, a_data ,label):
    net.train()
    pid_flag, model_type = model_isPid_type(params.model)
    N = int(math.ceil(len(pid_data) / params.batch_size))

    # Shuffle the data
    shuffled_ind = np.arange(qa_data.shape[0])
    ...
    for idx in tqdm(range(N)):
        ...
        a_one_seq = a_data[idx *
                           params.batch_size:(idx+1) * params.batch_size,:]

        if model_type in transpose_data_model:
            input_q = q_one_seq[:, :]  # Shape (bs, seqlen)
            input_qa = qa_one_seq[:, :] # Shape (bs, seqlen)
            target = a_one_seq[:, :]
            if pid_flag:
...
def test(net, params, optimizer, q_data, qa_data, pid_data, a_data,label):
    ...
    for idx in tqdm(range(N)):
        ...
        with torch.no_grad():
            if pid_flag:
                loss, pred, ct = net(input_q, input_qa, target, input_pid,test=lable)
            else:
